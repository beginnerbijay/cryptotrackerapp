  ...
  return (
    ...
    {search().slice('0','1').map((value,index)=>{
      return(
            ...
            <h2 className='text-xl'><span className='font-bold'>Current Price : </span>&#8377; {numberWithCommas(value.current_price)}</h2>
            </div>
            <div>
            <h2 className='text-xl'><span className='font-bold'>Market Cap : </span>&#8377; {numberWithCommas((value.market_cap/1000000).toFixed(0))}M</h2>
            <h2 className='text-xl'><span className='font-bold'>24 Change : </span><span className={value.price_change_percentage_24h>0?'text-[#11952e]':'text-[#f00f18]'}>{value.price_change_percentage_24h.toFixed(2)}%</span></h2>
            </div>
            <div>
            <h2 className='text-xl'><span className='font-bold'>24H High : </span>&#8377; {numberWithCommas(value.high_24h)}</h2>
            <h2 className='text-xl'><span className='font-bold'>24H Low : </span>&#8377; {numberWithCommas(value.low_24h)}</h2>
            </div>
